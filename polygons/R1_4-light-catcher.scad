...
function R1_4_partial(c,A,b,B,E) =
   let(C=180-B)
   let(D=360-A-E)
   let(a=1)
   let(e=1)
...
function f(x,P) =
      let(p=R1_4_partial(x,P[0],P[1],P[2],P[3]))
      peri_error(p);

function R1_4(A,b,B,E) =   
     let(P=[A,b,B,E])
     let(c=secant(0.1,2,P))
     let(p=R1_4_partial(c,A,b,B,E))
     shift(p,-1);

function R1_4_tiles(A,b,B,E,n,m) =
let(peri=R1_4(A,b,B,E))
