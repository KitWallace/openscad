...
tiles=R1_5_tiles(C,b,n,m);
//peri_report(peri);
//fill_tile(peri_to_tile(peri));
...
function R1_5_partial(C,b,c,d) =
    [[1,90],[b,180 - C/2],[c,C],[d,180-C],[d-c,90-C/2]];
    
function f(x,P) = 
    let(p = R1_5_partial(P[0],P[1],x[0],x[1]))
    peri_error(p);
...
function R1_5(C,b) =
    ...
    R1_5_partial(C,b,x[0],x[1]); 

function R1_5_tiles(C,b,n,m) =     
let(peri=R1_5(C,b))
