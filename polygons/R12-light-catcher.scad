...
colors=["red","green","blue","orange","pink","grey","black","purple"];
n=10;
m=10;
inset=0.3;
scale=4;
...
$fn=100;
tiles =  R12_tiles(C,n,m);
light_circle(90,2)
     fill_tiles(inset_group(scale_tiles(tiles,scale),inset));
 
// simplex works !
// however the scale is affected - need to fix
//           
...
function R12(C) =
     ...
     let(x=simplex([C],XV))
     R12_partial(C,x[0],x[1]);

function f(x,P) =
    let(b=x[0])
    let(e=x[1])
    let(C=P[0])
    let(p=R12_partial(C,b,e))
    let(t=peri_to_tile(p,true))
    path_error(t);

function R12_tiles(C,n,m) =
let(peri=R12(C))
